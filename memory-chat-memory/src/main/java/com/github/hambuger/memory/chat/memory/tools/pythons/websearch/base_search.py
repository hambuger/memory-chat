...
from websearch.exceptions import ConversationLimitException, DuckDuckGoSearchException, RatelimitException, TimeoutException
...
logger = logging.getLogger("base_search.DDGS")
...
class DDGS:
    ...
    def __init__(
        ...
        self.google_headers = {}
        self.google_headers["Referer"] = "https://www.google.com.hk/"
        self.google_client = pri.Client(
        ...
        self.baidu_headers = {}
        self.baidu_headers["Referer"] = "https://www.baidu.com/s?"
        self.baidu_client = pri.Client(
    ...
    def get_google_url(
        ...
        if self._exception_event.is_set():
            raise DuckDuckGoSearchException("Exception occurred in previous call.")
        try:
            resp = self.google_client.request(method, url, params=params, content=content, data=data)
        except Exception as ex:
            self._exception_event.set()
            if "time" in str(ex).lower():
                raise TimeoutException(f"{url} {type(ex).__name__}: {ex}") from ex
    ...
    def get_baidu_url(
        ...
        if self._exception_event.is_set():
            raise DuckDuckGoSearchException("Exception occurred in previous call.")
        try:
            resp = self.baidu_client.request(method, url, params=params, content=content, data=data)
        except Exception as ex:
            self._exception_event.set()
            if "time" in str(ex).lower():
                raise TimeoutException(f"{url} {type(ex).__name__}: {ex}") from ex
            raise DuckDuckGoSearchException(f"{url} {type(ex).__name__}: {ex}") from ex
        logger.info(f"_get_url() {resp.url} {resp.status_code} {len(resp.content)}")
        if resp.status_code == 200:
            return cast(bytes, resp.content)
        self._exception_event.set()
        if resp.status_code in (202, 301, 403):
            raise RatelimitException(f"{resp.url} {resp.status_code} Ratelimit")
        raise DuckDuckGoSearchException(f"{resp.url} return None. {params=} {content=} {data=} {resp.status_code=} {resp.content=}")


    async def get_common_url(
            self,
            method: str,
            url: str,
            params: Optional[Dict[str, str]] = None,
        ...
        if self._exception_event.is_set():
            raise DuckDuckGoSearchException("Exception occurred in previous call.")
        try:
            resp = self.common_client.request(method, url, params=params, content=content, data=data)
        except Exception as ex:
            self._exception_event.set()
            if "time" in str(ex).lower():
                raise TimeoutException(f"{url} {type(ex).__name__}: {ex}") from ex
    ...
    def _text_api(
        ...
        def _text_api_page(s: int) -> List[Dict[str, str]]:
            ...
            for row in page_data:
                href = row.get("u", None)
                if href and href not in cache and href != f"http://www.google.com.hk/search?q={keywords}":
                    cache.add(href)
